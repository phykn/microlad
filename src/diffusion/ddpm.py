...
class DDPMProcess:
    ...
    def add_noise(
        ...
        self._move(x_start.device)
        self._check_steps(t, x_start.shape[0])
    ...
    def renoise(
        ...
        """Applies one forward step from ``x_(t-1)`` back to ``x_t``."""
        self._move(x_previous.device)
        self._check_steps(t, x_previous.shape[0])

        if noise is None:
            noise = torch.randn_like(x_previous)
        if noise.shape != x_previous.shape:
            raise ValueError("noise must have the same shape as x_previous.")

        alpha = self._expand(self.alphas, t, x_previous.ndim)
        beta = self._expand(self.betas, t, x_previous.ndim)
    ...
    def predict_mean(
        ...
        self._move(x_t.device)
        self._check_steps(t, x_t.shape[0])
    ...
    def sample_step(
        ...
        *,
        ...
        mean = self.predict_mean(x_t, t, pred_noise)
    ...
    def _check_steps(self, t: torch.Tensor, batch_size: int) -> None:
        if batch_size <= 0:
            raise ValueError("batch_size must be positive.")
