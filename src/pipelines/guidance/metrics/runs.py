    ...
    *,
    ...
    for dimension in spatial_dimensions:
        ...
        for run_length in run_lengths:
            windows = oriented.unfold(-1, run_length, 1)
            survival = windows.prod(dim=-1)
            reduce_dimensions = tuple(
                index for index in range(survival.ndim) if index != 1
            )
            survival = survival.mean(dim=reduce_dimensions)
