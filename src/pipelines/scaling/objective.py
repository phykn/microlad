...
def slice_objective(
    ...
    *,
    ...
    probability_out = image.new_zeros(num_phases, height, width)
    ...
    stitched_probabilities = probability_out / weight_sum.clamp_min(
...
def batch_objective(
    ...
    *,
    ...
    probability_out = images.new_zeros(batch_size, num_phases, height, width)
    ...
    target_images = [
        as_anchor_image(target) if target is not None else None
        for target in anchor_targets
    ]
    mask_images = [
        as_anchor_image(mask) if mask is not None else None
        ...
        for slice_index, decoded_slice in enumerate(stitched):
            ...
            if mask_image is None:
                anchor_total, _ = anchor_loss(
                    ...
                    temperature=temperature,
                    weight=anchor_weight,
                    phase_probabilities=stitched_probabilities[slice_index],
                )
            else:
                anchor_total, _ = anchor_loss(
                    decoded_slice,
...
def decode_slices(
    ...
    *,
    ...
    window = (
        images.new_ones((tile_size, tile_size))
        if tile_overlap == 0
        else blend_window(
            tile_size,
            tile_size,
    ...
    for row, col in tile_grid(
        ...
        _, phase_probabilities = decode_latents(
            vae,
            mu,
            num_phases=num_phases,
        )
