...
def conductance_loss(
    ...
    *,
    ...
    actual_conductance = compute_conductance(
        values,
        solver=solver,
    ...
    target_conductance = build_phase_target(
        targets,
        num_phases=num_phases,
        device=actual_conductance.device,
        dtype=actual_conductance.dtype,
        label="conductance value",
    )
    require_finite("conductance targets", target_conductance)
    target_conductance = target_conductance.clamp(min=solver.low_cond, max=1.0)

    loss = weight * F.mse_loss(actual_conductance, target_conductance)

    stats = {
        "actual_conductance": actual_conductance.detach(),
        "target_conductance": target_conductance.detach(),
    }
