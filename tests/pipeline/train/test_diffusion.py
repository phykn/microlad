...
from src.pipeline.train import DiffusionTrainer
from src.pipeline.train.misc.distributed import unwrap_model
...
class DiffusionTrainerTest(unittest.TestCase):
    ...
    def test_train_shows_tqdm_progress(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("src.pipeline.train.diffusion.tqdm", FakeProgress):
                stats = trainer.train()
            trainer.close()
