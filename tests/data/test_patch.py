...
from src.data.patch import augment, crop, resize
...
class DataTransformTest(unittest.TestCase):
    def test_crop_returns_requested_region(self):
        ...
        with mock_patch("numpy.random.randint", side_effect=(1, 2)):
            cropped = crop(pixels, 2)

        np.testing.assert_array_equal(cropped, pixels[1:3, 2:4])

    def test_resize_preserves_categorical_labels(self):
        pixels = np.array([[0, 1], [2, 3]], dtype=np.uint8)

        resized = resize(pixels, 4)
    ...
    def test_resize_returns_copy_when_size_is_unchanged(self):
        pixels = np.array([[0, 1], [2, 3]], dtype=np.uint8)

        resized = resize(pixels, 2)
    ...
    def test_patch_ops_reject_non_uint8_labels(self):
