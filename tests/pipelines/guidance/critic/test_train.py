...
class CriticGuidanceTest(unittest.TestCase):
    ...
    def test_critic_progress_shows_loss_margin_and_gp(self):
        ...
        self.assertEqual(set(progress.postfixes[-1]), {"loss", "margin", "gp"})

    def test_critic_trains_all_steps_and_restores_best_step(self):
        critic = LatentCritic(2, base_ch=4)
        real = torch.randn(3, 4, 2, 16, 16)
        fake = torch.randn(2, 2, 16, 16, 16)
        config = CriticConfig(
        ...
        def metrics(score):
            value = torch.tensor(float(score))
            return {
                "validation_margin": value,
                "damage_margin": value,
                "shuffle_margin": value,
        ...
        with patch(
            "src.pipelines.guidance.critic.train.evaluate_critic",
            side_effect=[
                metrics(3),
                metrics(2),
                metrics(1),
        ...
        self.assertEqual(int(stats["critic_steps"]), 5)
        self.assertEqual(int(stats["critic_best_step"]), 1)
