...
from src.modeling.critic import sample_slices as sample_latent_slices
...
from src.pipelines.reconstruction.volume import decode_axis_probs, decode_volume_probs
...
class JointOptimizationTest(unittest.TestCase):
    ...
    def test_unlimited_decode_batch_disables_checkpointing(self):
        ...
        with patch(
            "src.pipelines.guidance.joint.optimize.decode_axis_probs",
    ...
    def test_global_fraction_uses_hard_forward_values(self):
        ...
        self.assertGreater(float(stats["history_global_fraction"]), 10.0)
    ...
    def test_joint_anchor_loss_changes_decoded_anchor_without_copying(self):
        ...
        self.assertEqual(stats["joint_candidate_delta_rms"].shape, torch.Size([3]))
        self.assertEqual(stats["joint_candidate_delta_max"].shape, torch.Size([3]))
        self.assertEqual(float(stats["joint_candidate_delta_rms"][0]), 0.0)
        self.assertEqual(float(stats["joint_candidate_delta_max"][0]), 0.0)
        self.assertTrue(torch.isfinite(stats["joint_base_std"]))
    ...
    def test_single_slice_prior_cycles_axes_across_steps(self):
        with patch(
            "src.pipelines.guidance.joint.optimize.sample_slices",
